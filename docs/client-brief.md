...
The Spanish below is the document that goes to the firm. Send it as it is: it
is written for lawyers, not developers, and contains no file paths, event names
or tool settings. The English at the end is our own record and stays here.

Keep it current: when the firm answers something, move it to "Answered
decisions" with the date and delete it from the Spanish.
...
>
> Este documento tiene tres partes: qué cambió en el sitio, qué van a notar en
> sus informes, y cinco decisiones que son suyas. La más importante tiene
> fecha: **1 de diciembre de 2026**, cuando entra en vigencia la Ley 21.719.
>
> ## 1. Qué cambió
>
...
>
> ## 2. Qué van a notar en los informes
>
> **Las conversiones en Google Ads van a bajar en los próximos días.** No es
> que menos personas los contacten: la misma cantidad de gente reserva y
> escribe por WhatsApp. Lo que cambia es cuántas de ellas Google puede ver,
> porque ahora sólo ve a quienes aceptan. Si sus campañas usan puja
> automática, Google tardará una o dos semanas en ajustarse; conviene no
> mover presupuestos en ese período. Y cualquier comparación de resultados
> debe partir del 22 de septiembre, no de los meses anteriores: son dos
> formas distintas de contar.
>
> **Quien rechaza no aparece en ningún informe.** Google ofrece estimar
> estadísticamente a esos visitantes, pero esa función sólo se activa en
> sitios con muchísimo más tráfico que el suyo. Los informes seguirán
> describiendo bien a quienes aceptan y las proporciones entre un paso y el
> siguiente siguen siendo fiables; lo que no podrán decirles es el número
> total de personas que consideraron contactarlos.
>
> **Quien rechaza la publicidad no recibe código de caso.** Ese código viaja en
> el mensaje de WhatsApp y en la planilla, y es lo que permite saber qué
> campaña trajo a cada persona. La consulta les llega igual —formulario, correo
> y fila en la planilla— pero sin código ni campaña asociada. Es la
> consecuencia directa de respetar lo que la persona eligió.
>
> ## 3. Decisiones que son suyas
>
...
> absolutamente nada a Google. Es un ajuste de configuración que toma minutos
> y se puede revertir. El costo es casi nulo, porque esos visitantes ya no
> aparecen en los informes hoy. Si prefieren hacer el cambio ahora, se hace de
> inmediato.
>
...
> rechazar. Si algún día la firma capta clientes en España o la Unión Europea,
> el botón de rechazo tiene que ir en el banner mismo.
>
...
For us, so the answer becomes a change without rereading the plan:

- **Decision 1** is a Tag Manager setting, not code: additional consent
  required on the Google tags. `docs/consent-mode-runbook.md` step 2 currently
  says to leave it off, on purpose, until the firm answers.
- **Decision 2** is one default per switch in `CookieSettingsModal.tsx`.
- **Decision 3** is one string in `CookieBanner.tsx`.
- **Decision 4** is `REPROMPT_BELOW_VERSION` in `src/lib/utils/consent.ts`,
  raised to 2.
- **Decision 5**: `docs/cookie-inventory.md` is the input for the policy pages.
