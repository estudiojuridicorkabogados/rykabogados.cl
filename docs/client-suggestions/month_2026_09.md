...
**First reading, 24 September 2026** (`RK · Recorrido desde el blog`, 28 days,
Active users). Of 123 people who landed on
`puedo-demandar-si-trabaje-sin-contrato`, **three** viewed a second page — two
more articles and **one** the workers' booking page. The next article, on
security cameras, had 85 in and two onward. About 97% of blog visits end on the
article they landed on. Caveats: the window straddles the consent go-live of 22
September, and a WhatsApp tap from the article is not a page, so it does not
show here. Neither explains away one booking-page visit from 123 readers of the
firm's most-read workers' article. Re-read after 21 October before raising it.
...
| On a phone, do they reach the box at the end? | `rk_cta_view` with `location` = `blog_post`, mobile, against mobile readers — an exact count, live since 24 September 2026. Before that, the `Lectura` tab's `75` column (the box sits at about 74% of the page) |
| Of those who see the box, how many use it? | `rk_cta_click` and `rk_conv_whatsapp` with `location` = `blog_post`, over `rk_cta_view` |
| How long do they stay? | `Por artículo`, average engagement time — read with depth: long and deep argues for a call to action at the end, long and shallow for one early |
