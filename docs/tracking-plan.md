...
Still open: the Ads account's time zone (Admin → Account settings) must be Santiago — Data Manager did not ask for one, so `conversion_time` is read in the account's zone; and after four weeks of real outcomes, whether `Cliente convertido` goes Primary.
