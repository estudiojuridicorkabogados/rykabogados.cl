...
**`rk_cta_view`, added 24 September 2026**, replaces the estimate with a count: the blog's contact box reports when it comes on screen, with `location` = `blog_post`. On a phone it fires only when the reader reaches the box — at 74% of the page on the most-read article, measured in a production build, which is why the `75` column was the right proxy. On desktop the box is a sticky sidebar and fires on arrival for nearly everyone, so read it on mobile. `location` is already a registered dimension and the `rk_*` rule forwards it, so no Tag Manager or Analytics change. It sits in `Lectura` itself rather than a tab of its own: the Event name filter there is **matches regex** `rk_scroll|rk_cta_view`, and `Event name` is in Columns above `percent_scrolled`, so each article shows the scroll marks beside an `rk_cta_view · (not set)` column — the phone readers who reached the box. Set up 24 September 2026, before the event was deployed: a free-form filter matches typed text, unlike the funnel's event picker, so it needs no data first, and the column appears once the first event is processed. `Device category` is in Rows, not a filter, so each article reads as mobile or desktop.
